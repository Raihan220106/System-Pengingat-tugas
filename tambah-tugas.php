...
// Validasi sederhana
    if (!empty($judul) && !empty($id_mk) && !empty($deadline) && !empty($status)) {
        // Query INSERT
        $id_user = $_SESSION['user_id']; // penting!
        $sql = "INSERT INTO tugas (id_user, id_mk, judul, deadline, Status) VALUES (?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("iisss", $id_user, $id_mk, $judul, $deadline, $status);
